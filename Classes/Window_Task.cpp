...
void Window_Task::init(void *data){
    
    m_tips = GUIReader::getInstance()->widgetFromJsonFile("ccs/Window_Task.json");
    ...
    for (auto btn :m_btns) {
        ...
        btn->addTouchEventListener(CC_CALLBACK_2(Window_Task::onBtnEvent, this));
    }
    m_autoCenter = false;
    ...
    for (int i=0; i<7; i++) {
        ...
        item->addTouchEventListener(CC_CALLBACK_2(Window_Task::onItemClick, this));
...
void Window_Task::onStartAnimOver(){
    HWindow::onStartAnimOver();
    firstShowItem(0);
}


void Window_Task::onBtnEvent(Ref* sender,Widget::TouchEventType type){
...
void Window_Task::onItemClick(cocos2d::Ref *sender, Widget::TouchEventType type){
    Widget* btn = dynamic_cast<Widget*>(sender);
    if (btn==nullptr) {
        return;
    }
...
void Window_Task::firstShowItem(int id){
...
void Window_Task::showItem(int id){
    setTocuhEnable(false);
...
Window_Task* Window_Task::instance = nullptr;
